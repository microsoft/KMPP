                 ...
                 gpointer user_data)
{
    ...
    if (handlerId == 0) {
        KEYISOP_trace_log_and_metric_error(NULL, 0, KeyIsoSolutionType_process, title, "handle-import-pfx", "ZeroId");
    }

    handlerId = g_signal_connect(interface, "handle-pfx-open", G_CALLBACK(KeyIso_on_handle_pfx_open), NULL);
    if (handlerId == 0) {
        KEYISOP_trace_log_and_metric_error(NULL, 0, KeyIsoSolutionType_process, title, "handle-pfx-open", "ZeroId");
    }

    handlerId = g_signal_connect(interface, "handle-pfx-close", G_CALLBACK(KeyIso_on_handle_pfx_close), NULL);
    if (handlerId == 0) {
        KEYISOP_trace_log_and_metric_error(NULL, 0, KeyIsoSolutionType_process, title, "handle-pfx-close", "ZeroId");
    }
    ...
    if (handlerId == 0) {
        KEYISOP_trace_log_and_metric_error(NULL, 0, KeyIsoSolutionType_process, title, "handle-rsa-private-encrypt-decrypt", "ZeroId");
    }

    handlerId = g_signal_connect(interface, "handle-ecdsa-sign", G_CALLBACK(KeyIso_on_handle_ecdsa_sign), NULL);
    if (handlerId == 0) {
        KEYISOP_trace_log_and_metric_error(NULL, 0, KeyIsoSolutionType_process, title, "handle-ecdsa-sign", "ZeroId");
    }

    handlerId = g_signal_connect(interface, "handle-create-self-sign-pfx", G_CALLBACK(KeyIso_on_handle_create_self_sign_pfx), NULL);
    if (handlerId == 0) {
        KEYISOP_trace_log_and_metric_error(NULL, 0, KeyIsoSolutionType_process, title, "handle-create-self-sign-pfx", "ZeroId");
    }

    handlerId = g_signal_connect(interface, "handle-replace-pfx-certs", G_CALLBACK(KeyIso_on_handle_replace_pfx_certs), NULL);
    if (handlerId == 0) {
        KEYISOP_trace_log_and_metric_error(NULL, 0, KeyIsoSolutionType_process, title, "handle-replace-pfx-certs", "ZeroId");
    }
    
    handlerId = g_signal_connect(interface, "handle-client-message", G_CALLBACK(KeyIso_on_handle_client_message), NULL);
    if (handlerId == 0) {
        KEYISOP_trace_log_and_metric_error(NULL, 0, KeyIsoSolutionType_process, title, "handle-client-message", "ZeroId");
    }

    handlerId = g_signal_connect(interface, "handle-get-version", G_CALLBACK(_on_handle_get_version), NULL);
    if (handlerId == 0) {
        KEYISOP_trace_log_and_metric_error(NULL, 0, KeyIsoSolutionType_process, title, "handle-get-version", "ZeroId");
    }      
...
int main(int argc, char *argv[])
{
    ...
    if (!KeyIsoP_create_pfx_secret(correlationId)) {
        KEYISOP_trace_log_error(correlationId, 0, title, "KeyIsoP_create_pfx_secret", "Failed");
    }
